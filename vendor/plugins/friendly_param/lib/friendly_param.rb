...
module ActiveRecord #:nodoc:
  module Acts #:nodoc:
    ...
    # A model using friendly_param may have a RESTful URL such as:
    #
    ...
    module FriendlyParam
      ...
      module ClassMethods
        ...
        #
        # friendly_param :title
        # 
        # other options are for customising iconv charmap translation
        # methods ie.
        #
        # friendly_param :name, :translation_to => 'utf-16'
        #
        # 
        def friendly_param(name_method=:name, options={})
          ...
          define_method :to_param do
            ...
            if respond_to?(name_method) && name = send(name_method)
              ...
              s = s.gsub(/^\s+|[^-_\s\w]|\s+$/, '').downcase.squeeze(' ').tr(' ','-')
